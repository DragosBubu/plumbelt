...
<?php if ( have_comments() ) : ?>
    <div class="title-border">
        <h3>
        <?php
            printf( _n( 'One comment on', '%1$s comments on &ldquo;%2$s&rdquo;', get_comments_number(), 'shape' ),
    ...
    <?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : ?>
        <nav role="navigation" id="comment-nav-above" class="site-navigation comment-navigation">
            ...
            <div class="nav-previous"><?php previous_comments_link( __( '&larr; Older Comments', 'shape' ) ); ?>
            </div>
            <div class="nav-next"><?php next_comments_link( __( 'Newer Comments &rarr;', 'shape' ) ); ?>
            </div>
        </nav>
    <?php endif; ?>
    ...
    comment_form(
        array(
            ...
            'label_submit' => esc_attr__( 'Submit', 'ti' ),
            'title_reply' => '<div class="title-border"><h3>' . esc_attr__( 'Leave a Reply' ,'ti' ) . '</h3></div>', 'title_reply_to' => esc_attr__( 'Leave a comment to %s', 'ti' )
        )
    );
    ?>
